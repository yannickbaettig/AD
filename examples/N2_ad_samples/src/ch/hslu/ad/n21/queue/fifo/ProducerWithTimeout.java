 ...
 */
package ch.hslu.ad.n21.queue.fifo;
...
public final class ProducerWithTimeout implements Runnable {
    ...
    private final BoundedFIFOQueue<Integer> queue;
    private final int maxRange;
    private long sum;

    /**
     * Erzeugt einen Produzent, der eine bestimmte Anzahl Integer-Werte produziert. Der Produzent
     * versucht zu speichern, und lässt es bei einem Timeout bleiben.
     * @param queue Queue zum Speichern der Integer-Werte.
     * @param max Anzahl Integer-Werte.
     */
    public ProducerWithTimeout(final BoundedFIFOQueue<Integer> queue, final int max) {
        this.queue = queue;
        this.maxRange = max;
        this.sum = 0;
    }
